...
class _BranchRegionMapScreenState extends State<BranchRegionMapScreen> {
  ...
  List<BranchRegionMap> _data = [];
  List<RegionMaster> _regions = [];

  final _formKey = GlobalKey<FormState>();
  final _orgCodeCtrl = TextEditingController(text: '101');
  ...
  List<BranchRegionMap> get _filtered {
    if (_search.isEmpty) return _data;
    final q = _search.toLowerCase();
    return _data.where((b) {
      return b.branchCode.toLowerCase().contains(q) || b.regionCode.toLowerCase().contains(q);
    }).toList();
  }

  void _go(MFView v, [BranchRegionMap? r]) {
    setState(() {
      _view = v;
      _sel = r;
      _delConfirmed = false;
  ...
  void _resetForm() {
    _orgCodeCtrl.text = '101';
    _branchCodeCtrl.clear();
    _regionCodeCtrl.text = _regions.isNotEmpty ? _regions.first.regionCode : '';
    _statusValue = true;
  }

  void _loadRecord(BranchRegionMap r) {
  ...
  Future<void> _saveRecord(bool isEdit) async {
    if (_formKey.currentState!.validate()) {
      if (_regionCodeCtrl.text.isEmpty) {
        _toast('Please select a Region Code', isError: true);
        return;
      }
      ...
      final record = BranchRegionMap(
        ...
        regionCode: _regionCodeCtrl.text,
        status: _statusValue,
      );
      ...
      try {
        if (isEdit) {
          ...
          showSuccessDialog(context, 'Branch Region Map update submitted for authorization.', onConfirm: () => _go(MFView.list));
        } else {
          await BranchRegionMapApiService.createMap(record);
          showSuccessDialog(context, 'Branch Region Map submitted for authorization.', onConfirm: () => _go(MFView.list));
        }
      } catch (e) {
  ...
  Future<void> _confirmDelete() async {
    if (_sel != null) {
      setState(() => _isLoading = true);
      try {
        await BranchRegionMapApiService.deleteMap(_sel!.branchCode);
        showSuccessDialog(context, 'Mapping deleted successfully', onConfirm: () => _go(MFView.list));
      } catch (e) {
        _toast(e.toString().replaceFirst('Exception: ', ''), isError: true);
        setState(() => _isLoading = false);
      }
  ...
  Widget _list() {
    ...
    final pagedList = list.isEmpty ? <BranchRegionMap>[] : list.sublist(startIndex, endIndex);

    final int activeCount = _data.where((e) => e.status).length;
    final int inactiveCount = _data.length - activeCount;

    return SingleChildScrollView(
      padding: const EdgeInsets.all(20),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _pageHeader(title: 'Branch Region Map'),
        ...
        Row(mainAxisAlignment: MainAxisAlignment.end, children: [
          ...
          _fBtn('New Map', Icons.add_rounded, const Color(0xFF1E3050), Colors.white, const Color(0xFF1E3050), onTap: () => _go(MFView.create)),
        ]),
        const SizedBox(height: 16),
        _card(child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          Container(
            ...
            child: Row(children: [
              Expanded(flex: 3, child: _colHdr('BRANCH CODE')),
              Expanded(flex: 3, child: _colHdr('REGION CODE')),
            ...
            ListView.separated(
              ...
              itemBuilder: (_, i) {
                ...
                return Container(
                  ...
                  child: Row(children: [
                    Expanded(flex: 3, child: Text(r.branchCode, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF1E293B)))),
                    Expanded(flex: 3, child: Text(r.regionCode, style: const TextStyle(fontSize: 13, color: Color(0xFF1E293B)))),
                    Expanded(flex: 2, child: Align(alignment: Alignment.centerLeft, child: _statusBadge(r.status))),
  ...
  Widget _form({bool isEdit = false, bool isView = false}) {
    final regionItems = _regions.map((r) => {'id': r.regionCode, 'name': r.regionName}).toList();
    ...
    return Column(children: [
      Expanded(
        child: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            _pageHeader(
              title: isView ? 'View Branch Region Map' : (isEdit ? 'Edit Branch Region Map' : 'Create Branch Region Map'),
              actions: [
            ...
            if (isEdit)
              Container(
                ...
                child: const Row(children: [
                  ...
                  Expanded(child: Text('Changes will be submitted for authorization.', style: TextStyle(fontSize: 13, color: Color(0xFFB45309)))),
                ]),
              ),
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Wrap(spacing: 24, runSpacing: 24, children: [
                  ...
                  SizedBox(width: 300, child: MFApiDropdownField(
                    label: 'Region Code', icon: Icons.location_city, required: !isView,
                    items: regionItems,
                    displayKeys: const ['id', 'name'],
                    selectedItem: _regionCodeCtrl.text.isNotEmpty ? regionItems.firstWhere((e) => e['id'] == _regionCodeCtrl.text, orElse: () => {'id': _regionCodeCtrl.text, 'name': ''}) : null,
                    onChanged: (v) { if (v != null) setState(() => _regionCodeCtrl.text = v['id']); },
                    enabled: !isView,
                  )),
                ]),
      ...
      Container(
        ...
        child: Row(children: [
          ...
          if (!isView) ...[
            const SizedBox(width: 12),
            _fBtn(isEdit ? 'Submit Update' : 'Submit Record', isEdit ? Icons.save_rounded : Icons.check_circle_rounded, const Color(0xFF1E3050), Colors.white, const Color(0xFF1E3050), onTap: _isLoading ? null : () => _saveRecord(isEdit)),
          ]
        ]),
      ),
    ]);
